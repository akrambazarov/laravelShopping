...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if (!in_array($_FILES['picture']['type'], $types))
		die('<p> Выберите фотографию. <a href="?">Попробовать другой файл?</a></p>');
	...
	function resize($file, $type = 1, $rotate = null, $quality = null)
	{
		...
		elseif ($file['type'] == 'image/jpg')
			$source = imagecreatefromjpg($file['tmp_name']);
		else
			return false;
	...
	if (!@copy($tmp_path . $name, $path . $name))
		...
		echo '<p>Загрузка прошла удачно <a href="' . $path . $_FILES['picture']['name'] . '">Посмотреть</a>.</p>'.$name;
